// 수정된 로직으로 10가지 새로운 여행 시나리오 테스트
// In-Flight 감지 개선 및 Option 표시 로직 확인
...
function calculateJetLagPlan(scenario) {
    ...
    const isInFlightA = (fastingStartDestA < arrivalDate && !isPreFlightA);

    // Option B 표시 여부 (수정된 로직)
    const shouldShowOptionB = isPreFlightA || isInFlightA || isTightConnectionA;

    let strategyAName;
    if (isPreFlightA) {
        strategyAName = shouldShowOptionB ? "🚀 Option A: Start Before Departure" : "🚀 Start Before Departure";
    } else if (isInFlightA || isTightConnectionA) {
        strategyAName = shouldShowOptionB ? "🚀 Option A: Start In-Flight" : "🚀 Start In-Flight";
    } else {
        strategyAName = "🚀 Your Reset Plan";
    }
...
output += "JET LAG RESET CALCULATOR - 수정된 로직 테스트\n";
output += "=".repeat(80) + "\n\n";
...
output += "   - In-Flight: 금식이 출발~도착 사이 시작 (목적지 시간 기준)\n";
output += "   - After Arrival: 금식이 도착 2시간 이후 시작\n";
output += "5. Option 표시 로직 (개선됨):\n";
output += "   - Option B 표시: Before Departure OR In-Flight OR Tight Connection일 때\n";
output += "   - Option B 없을 때: 'Option A' 대신 'Your Reset Plan'으로 표시\n";
output += "=".repeat(80) + "\n\n";
